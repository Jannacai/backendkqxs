...
class RedisManager {
    ...
    async publishSSEEvent(date, tinh, prizeType, prizeData, additionalData = {}) {
        try {
            ...
            await client.publish(`xsmn:${date}:${tinh}`, message);
    ...
    async updateLotteryData(date, tinh, prizeType, prizeData, additionalData = {}) {
        try {
            ...
            pipeline.hSet(`kqxs:xsmn:${date}:${tinh}`, prizeType, JSON.stringify(prizeData));
            pipeline.expire(`kqxs:xsmn:${date}:${tinh}`, 86400);
            ...
            pipeline.hSet(`kqxs:xsmn:${date}:${tinh}:meta`, 'metadata', JSON.stringify(metadata));
            pipeline.expire(`kqxs:xsmn:${date}:${tinh}:meta`, 86400);

            // Cập nhật latest data
            let latestData = await client.get(`kqxs:xsmn:${date}:${tinh}:latest`);
            ...
            pipeline.set(`kqxs:xsmn:${date}:${tinh}:latest`, JSON.stringify(fullData));
            pipeline.expire(`kqxs:xsmn:${date}:${tinh}:latest`, 86400);
    ...
    // Thêm method để lấy dữ liệu từ Redis (tương thích với xsmnLiveRoutes)
    async getData(key) {
        try {
            const client = await this.getClient();
